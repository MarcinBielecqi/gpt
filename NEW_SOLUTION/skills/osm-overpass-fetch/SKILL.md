# Skill: OSM Overpass Fetch

## Cel

Pobiera obiekty OSM z Overpass i zapisuje je do kanonicznej bazy `canon.sqlite`.
...
| `--key` | Klucz tagu OSM. |
| `--value` | Wartość tagu OSM. |
| `--element-types` | `node`, `way`, `relation` lub `nwr`. |
...
runs/<run_id>/skills/osm_overpass_fetch/run.sqlite
```
...
| `artifact_type` | Opis |
|---|---|
| `osm_features_import` | Raport importu OSM do canon DB. |
