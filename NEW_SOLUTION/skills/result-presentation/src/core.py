...
def add_domain_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--analysis-dir", help="Directory with analysis JSON files. Default: ./analysis from the command working directory.")
...
def _start_map(args: argparse.Namespace, ctx) -> dict[str, Any]:
    ...
    server_py = Path(__file__).resolve().parent / "map_server_compact.py"
    canon_db = _absolute(ctx.canon_db)
    bus_db = _absolute(ctx.bus_db)
    analysis_dir = _absolute(args.analysis_dir) if args.analysis_dir else _absolute("analysis")
    ...
    return {
        ...
        "server": "result-presentation/src/map_server_compact.py",
